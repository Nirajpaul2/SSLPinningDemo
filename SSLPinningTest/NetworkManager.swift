...
class NetworkManager {
    ...
    public func testPinning() {
        ...
        let serverTrustPolicies: [String: ServerTrustPolicy] = [
            "www.purplle.com": .pinCertificates(
        ...
        NetworkManager.networkManager.manager!.request("https://www.purplle.com/product").response { response in
            if response.error == nil {
                print("Success")
                print(response.response?.statusCode)
            } else {
    ...
    private func getCertificates() -> [SecCertificate] {
        let url = Bundle.main.url(forResource: "purplle", withExtension: "cer")!
